...
public class BuzzLightyear extends AdvancedRobot {
	...
	public void run() {
		...
		nextDestination = lastPosition = myPos = new Point2D.Double(getX(),
				getY());
	...
	public void doMovementAndGun() {
		...
		if (getGunTurnRemaining() == 0 && myEnergy > 1) {
			setFire(Math.min(Math.min(myEnergy / 6d, 1300d / distanceToTarget),
					target.energy / 3d));
		}

		setTurnGunRightRadians(Utils.normalRelativeAngle(calcAngle(target.pos,
		...
		if (distanceToNextDestination < 15) {
			...
			double addLast = 1 - Math
					.rint(Math.pow(Math.random(), getOthers()));
	...
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		Enumeration enumer = enemies.elements();
		while (enumer.hasMoreElements()) {
			microEnemy en = (microEnemy)enumer.nextElement();
	...
	public void onScannedRobot(ScannedRobotEvent e) {
		...
		en.live = true;
		en.pos = calcPoint(myPos, e.getDistance(),
	...
	// - math
	// ------------------------------------------------------------------------------------------------------------------------------------
	private static Point2D.Double calcPoint(Point2D.Double p, double dist,
			double ang) {
